...
async function checkRedOrder() {
    ...
    if (order) {
        ...
        if (!ack) {
            activeRedOrderId = order.id;
            document.getElementById('red-order-text').innerText = order.message;
            document.getElementById('modal-red-order').style.display = 'flex';
        }
        ...
        if (isPresGlobal) { document.getElementById('order-status-text').innerText = `⚪ Нет активных приказов.`; document.getElementById('order-status-text').style.color = "gray"; }
    }
}
async function publishRedOrder() {
    const text = document.getElementById('admin-order-text').value.trim();
    if (!text) return alert("Введите текст приказа!");
    await supabaseClient.from('red_orders').update({ is_active: false }).eq('is_active', true); // Отключаем старые
    await supabaseClient.from('red_orders').insert([{ message: text }]); // Создаем новый
    alert("Красный Приказ издан! Все тренеры увидят его при входе.");
    document.getElementById('admin-order-text').value = '';
    checkRedOrder();
}
async function disableRedOrder() {
    await supabaseClient.from('red_orders').update({ is_active: false }).eq('is_active', true);
    alert("Приказ отменен."); checkRedOrder();
}
async function acknowledgeOrder() {
    if(!activeRedOrderId) return;
    await supabaseClient.from('order_acks').insert([{ order_id: activeRedOrderId, profile_id: currentUserId }]);
    document.getElementById('modal-red-order').style.display = 'none';
}
...
async function loadPoll() {
    ...
    let vMap = {}; currentPollOptions.forEach((_,i) => vMap[i] = 0); let total = 0; let userVotedIdx = -1;
    ...
    if(userVotedIdx !== -1 || currentPollOptions.length === 0) {
        ...
        h += `<p style="font-size: 11px; color: #888; text-align: center; margin: 5px 0 0 0;">Всего голосов: ${total}</p>`; cont.innerHTML = h;
    } else {
        let h = ''; currentPollOptions.forEach((opt, idx) => { h += `<button class="poll-option" onclick="castVote(${idx})">${opt}</button>`; }); cont.innerHTML = h;
    }
}
...
// === БАЗА ТУРНИРОВ, МАНДАТНАЯ КОМИССИЯ И БАЛЛЫ ===
...
async function loadTournamentsForPoints() {
    ...
    globalTournaments.forEach(t => { 
        ...
        if(isPresGlobal) { actionBtns += `<button class="btn-link" style="padding:4px 8px; font-size:10px; margin-top:5px; width:100%;" onclick="viewApplications('${t.id}', '${t.title}')">Смотреть список</button><button class="btn-del" style="margin-top:5px; width:100%;" onclick="deleteTournament('${t.id}')">🗑 Удалить турнир</button>`; }
...
async function saveNewTournament() { const t = document.getElementById('ct-title').value, l = document.getElementById('ct-level').value, d = document.getElementById('ct-date').value; if(!t || !d) return alert("Заполните все поля!"); await supabaseClient.from('tournaments').insert([{title: t, level: l, event_date: d}]); document.getElementById('admin-btn-add-tour').style.display = 'inline-block'; closeModal('modal-create-tournament'); loadTournamentsForPoints(); }
...
async function saveApplication() { const tourId = document.getElementById('apply-tour-id').value; const checkboxes = document.querySelectorAll('.apply-checkbox'); let selectedIds = []; checkboxes.forEach(cb => { if(cb.checked) selectedIds.push(cb.value); }); await supabaseClient.from('tournament_applications').delete().eq('tournament_id', tourId).eq('profile_id', currentUserId); if(selectedIds.length > 0) { let inserts = selectedIds.map(sid => ({ tournament_id: tourId, student_id: sid, profile_id: currentUserId })); await supabaseClient.from('tournament_applications').insert(inserts); } closeModal('modal-apply'); alert("Заявка успешно отправлена!"); }
...
async function viewApplications(tourId, tourTitle) {
    ...
    const { data: apps } = await supabaseClient.from('tournament_applications').select('*, students(*), profiles(full_name)').eq('tournament_id', tourId); currentTournamentApps = apps || []; 
    let h = '';
...
function exportAppsToExcel(tourTitle) {
    if(currentTournamentApps.length === 0) return alert("Список пуст!");
    let csvContent = '\uFEFFФИО;Возраст;Вес;Кю/Дан;Разряд;Тренер\n'; 
...
async function loadRankings() {
    ...
    sortedSt.forEach((x,i) => fHtml+=`<tr><td><strong>${i+1}</strong></td><td><strong>${x[0]}</strong></td><td><strong>${x[1]} б.</strong></td>${isPresGlobal?`<td><button class="btn-del" onclick="deleteResult('${stIds[x[0]]}')">🗑 Сброс</button></td>`:''}</tr>`); document.getElementById('full-ranking-body').innerHTML = fHtml;
...
async function openStudentProfile(studentId) {
    ...
    let gold = 0, silver = 0, bronze = 0, totalPts = 0; if(results) { results.forEach(r => { if(r.place == 1) gold++; if(r.place == 2) silver++; if(r.place == 3) bronze++; totalPts += r.points; }); }

    // Загрузка Истории Экзаменов (Летопись)
    ...
    if(history && history.length > 0) {
        history.forEach(h => { histHtml += `<div class="history-item"><strong>${h.kyu}</strong><span>🗓 ${h.exam_date} | Принимал: ${h.examiner_name}</span></div>`; });
    } else { histHtml = '<p style="font-size:12px; color:#888; font-style:italic;">История экзаменов пуста</p>'; }
...
// === АТТЕСТАЦИЯ (С ЗАПИСЬЮ В ЛЕТОПИСЬ) ===
...
async function saveExamResult() { 
    ...
    // 1. Обновляем текущий пояс у студента
    await supabaseClient.from('students').update({ current_kyu: newKyu, last_exam_date: date }).eq('id', id); 
    
    // 2. Пишем в Летопись
    const { data: profile } = await supabaseClient.from('profiles').select('full_name').eq('id', currentUserId).single();
    const examiner = profile && profile.full_name ? profile.full_name : 'Инструктор Федерации';
...
async function loadProfile(userId) {
    ...
    if (data) {
        ...
        checkRedOrder(); // Проверяем приказ после загрузки профиля
    }
